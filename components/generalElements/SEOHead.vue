...
const defaultImage =
  imagePrefix + img.getSizes(imageURL, { width: 1200, height: 630 }).src;
const twitterImageSrc =
  imagePrefix + img.getSizes(imageURL, { width: 1600, height: 900 }).src;
const description =
...
const facebook =
  wooNuxtSEO?.find((item) => item.provider === "facebook") ?? null;
